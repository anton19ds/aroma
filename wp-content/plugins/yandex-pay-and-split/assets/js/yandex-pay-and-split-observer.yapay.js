...
const getForm = () => {
    var _a;
    return ensure_ensure((_a = document.querySelector('.checkout.woocommerce-checkout')) !== null && _a !== void 0 ? _a : document.querySelector('.wc-block-components-form.wc-block-checkout__form'), {
...
;// ./browser/lib/guards.ts

function isYaPayPaymentMethod(income) {
    return typeof income === 'string' && Object.keys(PAYMENT_METHODS_DICT).includes(income);
}

;// ./browser/ya-pay-cart-observer.ts




jQuery(($) => __awaiter(void 0, void 0, void 0, function* () {
    const onPaymentMethodUpdate = () => __awaiter(void 0, void 0, void 0, function* () {
        const form = getForm();
        const formData = new FormData(form);
        const method = formData.get('payment_method');
        const totalAmount = formData.get('yandex_pay_total_amount');
        try {
            if (isYaPayPaymentMethod(method)) {
                yield window.YaPayWC.update({
                    availablePaymentMethods: [PAYMENT_METHODS_DICT[method]],
                    totalAmount: typeof totalAmount === 'string' ? totalAmount : undefined,
                });
            }
            else {
    ...
    const initEvents = () => __awaiter(void 0, void 0, void 0, function* () {
        $(document.body)
            .on('payment_method_selected', onPaymentMethodUpdate)
            .on('init_checkout', onPaymentMethodUpdate)
            .on('updated_checkout', onPaymentMethodUpdate);
        yield onPaymentMethodUpdate();
    });
    if (window.YaPay) {
        yield initEvents();
    }
